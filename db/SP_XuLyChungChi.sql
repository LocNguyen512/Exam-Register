...
-- Kiểm tra xem thí sinh thi các loại dgnl nào
/*
DECLARE @MA_TS CHAR(6) = 'TS1800'; -- Thay bằng mã thí sinh bạn muốn kiểm tra
...
SELECT DISTINCT 
    ...
    LT.NGAYTHI,
    LT.GIOTHI,
    LD.TENLOAI AS TEN_KY_THI
FROM 
    THI_SINH TS
JOIN 
    CHI_TIET_DANG_KY CTDK ON TS.MA_TS = CTDK.MA_TS
JOIN 
    CHI_TIET_LICH_THI CTLT ON CTDK.MA_PHONG = CTLT.MA_PHONG AND CTDK.MA_LICH = CTLT.MA_LICH
JOIN 
    LICH_THI LT ON CTLT.MA_LICH = LT.MA_LICH
JOIN 
    LOAI_DGNL LD ON LT.MA_LOAI = LD.MA_LOAI
WHERE 
    TS.MA_TS = @MA_TS;
